import { StoreInterface, NEW_LICENSE} from "./context/userContext"
...
export const stateReducer = (store:StoreInterface, action:ACTIONTYPE) => {
    ...
    const actionMap:ACTIONMAP = {
        ...
        businessInfo: () => ({ ...store, businessInfo: {...store.businessInfo, [datakey]: payload} }),
        state: () => ({...store, state: {...store.state, [datakey]: payload } }),
        currentLicense: () => ({...store,  currentLicense: {...store.currentLicense, [datakey]: payload} }),
        addLicense: () => {
        ...
        deleteLicense: () => {
            const filteredLicenses = store.licenses.filter(license => license.id !== payload);
            return {...store, licenses: filteredLicenses}
        },
        editLicense: () => {

            const {id, ...rest} = payload as License;
            const updatedStore = {...store, state:{...store.state, [datakey]: 'editing', currentEditingLicenseId: id}, currentLicense: rest };
        ...
        switchSurveyToggle: () => ({...store, survey: {...store.survey,  [datakey]: payload} }),
        checkFilling: () => {
            //@ts-ignore
            const isAllFieldsFilled = payload.every((key: string) => store[datakey][key].length > 0 ) ? 'approved' : 'filling'

            return ({...store, state: {...store.state, fillState: {...store.state.fillState, [datakey]: isAllFieldsFilled} }})
    
        },
        checkToggles: () => {

            const isToggleSwiped = payload.some((value:boolean) => value === true) ? 'approved' : 'filling';
            return ({...store, state: {...store.state, fillState: {...store.state.fillState, [datakey]: isToggleSwiped} }})
        },
        fillingInterrupted: () => {
            return ({...store, state: {...store.state, fillState: {...store.state.fillState, [datakey]: 'invalid'} }})
        },
        modalToggle: () => {
