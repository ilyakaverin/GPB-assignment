...
import { UserContext, ContextInterface, NEW_LICENSE } from '../../context/userContext';
...
const LicenseCard = ({ license }:LicenseCardProp) => {
    ...
    return (
        ...
        <span>{license.documentId}, {license.typeOfDocument} </span>
        ...
        <span>{license.dateOfIssue} - {license.expiresAt}</span>
        </div>
        <div className={style.main__container_card_buttons}>
            <OutputCardButton img="edit.svg" name="Редактировать" click={() => {
                dispatch({type: 'editLicense', datakey: 'licenseMode', payload: license })
            }} />
            <OutputCardButton img="delete.svg" name="Удалить" click={() => {
                dispatch({type: 'deleteLicense', datakey: 'licenses', payload: license.id})
                dispatch({type: 'clearLicenseForm', datakey: 'newLicense', payload:  null});
            }} />
        </div>
